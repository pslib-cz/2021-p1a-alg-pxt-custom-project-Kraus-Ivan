...
scene.onOverlapTile(SpriteKind.Player, assets.tile`wood`, function (sprite, location) {
    ...
    if (currentLevel == 4 && (bobr.tileKindAt(TileDirection.Left, assets.tile`wood`) || bobr.tileKindAt(TileDirection.Right, assets.tile`wood`))) {
...
scene.onOverlapTile(SpriteKind.Player, assets.tile`active_black`, function (sprite, location) {
    if (currentLevel == 5) {
...
game.onUpdateInterval(1000, function () {
    if (currentLevel == 5 && fightScene == true) {
        for (let value of sprites.allOfKind(SpriteKind.Enemy)) {
            scene.followPath(value, scene.aStar(value.tilemapLocation(), mySprite.tilemapLocation()), 50)
        }
    }
})
forever(function () {
